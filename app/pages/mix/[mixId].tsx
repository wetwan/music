...
import { musics, yourMix } from "@/constants/music";
  ...
  useEffect(() => {
    if (typeof mixId === "string" && mixId.trim()) {
      const filteredMusicList = musics.filter(
        (item) => Array.isArray(item.mix) && item.mix.includes(mixId)
      );

      const playlistDetails = yourMix.find(
        (item) => item.name === mixId // safer than comparing by name
      );
      ...
      console.error("mixId is missing or not a valid string");
    }
  }, [mixId, setPlaylist]);
  return (
    <>
